...
// TODO actions to modify GenericUserEnv
...
func (s *BasicUser[B]) ActRandomTxValue(t Testing) {
	...
	s.txValue = big.NewInt(s.rng.Int63())
}
...
// TODO more actions to randomize tx contents
...
func (s *CrossLayerUser) ActDeposit(t Testing) {
	...
	tx, err := s.L1.env.Bindings.OptimismPortal.DepositTransaction(&s.L1.txOpts, toAddr, s.L2.txValue, s.L2.txOpts.GasLimit, isCreation, s.L2.txCallData)
...
func (s *CrossLayerUser) ActStartWithdrawal(t Testing) {
	...
	require.NoError(t, err, "sending initiate withdraw tx")
